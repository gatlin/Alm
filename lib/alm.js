...
}(global.window, function(Public) {

/**
 * Alm.js
 *
 * An ECMAScript library promoting a functional reactive programming style.
 */

'use strict';

let loeb = require('./loeb.js');
let instance = loeb.instance,
    Functor = loeb.Functor,
    Monad = loeb.Monad,
    constant = loeb.constant,
    id = loeb.id,
    guid_factory = loeb.guid_factory;
...
function setupVdom(alm) {
     ...
     * enough.
     *
     * However, re-rendering the actual DOM each time is problematic for the
     * following reasons:
     *
